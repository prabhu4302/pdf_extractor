
import os
from flask import Flask, render_template, request, redirect, flash, make_response, session
import fitz  # PyMuPDF
import re
from reportlab.pdfgen import canvas
from io import BytesIO

app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = 'uploads'
app.secret_key = 'your-secret-key-here'

APPROVED_COURSES = {
    "Don't Feed The 'ish": "DFT",
    "Mandatory - Working In Partnership with BT": "BT-PART",
    "Mandatory - Living up to Our Commitments RCM Training": "RCM"
}

def is_valid_certificate(text):
    """Check basic certificate structure"""
    return all(
        elem in text for elem in [
            "Certificate of completion",
            "has completed",
            "on"
        ]
    )


def verify_certificate(pdf_path):
    doc = fitz.open(pdf_path)
    text = "".join([page.get_text() for page in doc])

    if not is_valid_certificate(text):
        return None

    course_match = re.search(r"has completed\s+-+\s+## (.*?)\s+on", text, re.DOTALL) or                    re.search(r"has completed\s+(.*?)\s+on", text, re.DOTALL)

    course_title = course_match.group(1).strip() if course_match else None

    if not course_title or course_title not in APPROVED_COURSES:
        return None

    date_match = re.search(r"on\s+(\d{1,2}/\d{1,2}/\d{4})", text)

    return {
        "course_title": course_title,
        "course_code": APPROVED_COURSES[course_title],
        "completion_date": date_match.group(1) if date_match else "Date not available"
    }

def generate_verification_pdf(verified_courses):
    buffer = BytesIO()
    p = canvas.Canvas(buffer)
    p.setFont("Helvetica-Bold", 16)
    p.drawString(100, 800, "BT Certificate Verification Report")
    p.setFont("Helvetica", 12)
    p.drawString(100, 770, "Your BT certificates have been successfully verified.")
    y = 730
    for course in verified_courses:
        p.drawString(100, y, f"Course: {course['course_title']} ({course['course_code']})")
        p.drawString(100, y-20, f"Completed on: {course['completion_date']}")
        y -= 50
    p.setFont("Helvetica-Oblique", 10)
    p.drawString(100, 50, "Generated by BT Certificate Verification System")
    p.showPage()
    p.save()
    buffer.seek(0)
    return buffer

@app.route('/', methods=['GET', 'POST'])
def index():
    if request.method == 'POST':
        files = [
            request.files.get('pdf_file_1'),
            request.files.get('pdf_file_2'),
            request.files.get('pdf_file_3')
        ]

        verified_courses = []
        rejected_files = []

        for file in filter(None, files):
            if not file.filename.lower().endswith('.pdf'):
                rejected_files.append(file.filename)
                continue

            try:
                file_path = os.path.join(app.config['UPLOAD_FOLDER'], file.filename)
                file.save(file_path)
                result = verify_certificate(file_path)
                if result:
                    result["filename"] = file.filename
                    verified_courses.append(result)
                else:
                    rejected_files.append(file.filename)
            except Exception as e:
                rejected_files.append(file.filename)

        session['verified_courses'] = verified_courses
        session['rejected_files'] = rejected_files

        return redirect('/results')

    return render_template('index.html')

@app.route('/results')
def results():
    verified_courses = session.get('verified_courses', [])
    rejected_files = session.get('rejected_files', [])
    return render_template('results.html', verified_courses=verified_courses, rejected_files=rejected_files)

@app.route('/download')
def download():
    verified_courses = session.get('verified_courses', [])
    if not verified_courses:
        flash("No verified courses found", "error")
        return redirect('/')

    pdf_buffer = generate_verification_pdf(verified_courses)
    response = make_response(pdf_buffer.getvalue())
    response.headers['Content-Type'] = 'application/pdf'
    response.headers['Content-Disposition'] = 'inline; filename=bt_certificate_verification.pdf'
    return response

if __name__ == '__main__':
    os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
    app.run(host='0.0.0.0', port=5000, debug=True)
